...
# case, where ~/.bashrc adds conda to PATH but the current shell hasn't
# picked it up.
...
# --- 2. Env ------------------------------------------------------------------
# On Linux `pip install torch` pulls the default (CUDA-enabled) wheels, which
# is what GPU users want; it still runs on CPU-only machines.
